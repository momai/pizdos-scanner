...
Инструмент для сканирования диапазонов IP-адресов: проверяет доступность хостов по ICMP и TCP, собирает живые адреса и отклонённые (rejected) соединения.

**Что умеет:**
- Сканировать `/24` подсети из Xray/V2Ray `geoip.dat`, CIDR-файлов или списка адресов
- Пробовать хосты по ICMP и TCP (порты задаются в конфиге), включая TLS/SNI
- Быстрый ICMP-only скан + последующий TCP-скан найденных живых IP
- Параллельный скан нескольких `/24` одновременно
- Сохранять результаты постепенно (CSV, JSONL, txt-списки alive/rejected), возобновлять прерванный скан
- Контролировать доступность внешнего endpoint после каждой подсети (автостоп или ротация IP)
- Остановить скан, как только whitelist-цель стала достижима

![TUI-дашборд сканера](docs/screenshots/tui-dashboard.png)

## Быстрый старт, one-liner установка 
...
В репозитории лежат готовые списки CIDR (по одной сети на строку), собранные из анонсируемых префиксов RIPEstat по ASN провайдеров. Сканер сам разворачивает их в `/24`. Перекрывающиеся CIDR схлопываются при генерации.
