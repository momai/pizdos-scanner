...
**Возможности:**
- Источник подсетей: `geoip.dat` (Xray/V2Ray), CIDR-файл или произвольный список адресов
- Проверка хостов по ICMP и TCP, порты и TLS/SNI настраиваются в конфиге
- Параллельное сканирование нескольких `/24` одновременно
- Результаты пишутся сразу (CSV, JSONL, txt-списки alive/rejected), скан можно прервать и продолжить
- Периодическая проверка внешнего endpoint: автостоп или ротация IP при недоступности
- Автостоп, как только whitelist-адрес стал достижим
