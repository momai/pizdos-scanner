...
CSV содержит сводку по подсети: `icmp_hosts`, `active_hosts` и колонки по каждому TCP-порту (`tcp_80_hosts`, `tcp_443_hosts`).

JSONL содержит расширенную запись с компактным `probe`: диапазоны последних октетов для `icmp`, `tcp_ports` и `dead`, например `tcp_ports: {"80": ["6", "8-12"], "443": ["6", "8-12"]}`.
